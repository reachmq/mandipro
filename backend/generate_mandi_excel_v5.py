"""
Mandi Master Excel Generator - VERSION 5.0
Fixed: BF_DISC as expense, ZAKAT as liability (provision/paid)
"""
...
def create_v5(path):
    ...
    settings = [
        ...
        ["Opening Bank", 0],
        ["", ""],
        ["ZAKAT Opening Payable", 0],  # Opening Zakat liability
    ]
    for r, (lbl, val) in enumerate(settings, 2):
        ...
        c.fill = INPUT_FILL
        if "Cash" in str(lbl) or "Bank" in str(lbl) or "ZAKAT" in str(lbl):
    ...
    ws.cell(row=2, column=1, value="This feeds: Bepaari Ledger, Dukandar Ledger, Commission").font = Font(italic=True, size=9)
    ...
    ws.cell(row=2, column=1, value="BEP expenses in OUT cols. ZAKAT: PROVISION=provision, PAID=payment").font = Font(italic=True, size=9)
    
    for c, h in enumerate(["Sr", "Date", "Type", "Sub-Type", "Party", "Particulars", "Cash OUT", "Bank OUT", "Cash IN", "Bank IN"], 1):
        ws.cell(row=3, column=c, value=h)
    hdr(ws, 3, 1, 10)
    
    # Main types: BEPAARI, DUKANDAR, CAPITAL, LOAN, ADVANCE, EXPENSE, ZAKAT, OTHER
    ...
    # Sub-types based on main type
    # BEPAARI: PAYMENT, MOTOR, BHUSSA, GAWALI, CASH_ADV, OTHER
    # DUKANDAR: RECEIPT
    # CAPITAL: TAKEN, WITHDRAWN
    # LOAN: TAKEN, REPAID
    # ADVANCE: GIVEN, RECEIVED
    # EXPENSE: MANDI, TRAVEL, FOOD, SALARY, BF_DISC, JB_PAID, OTHER
    # ZAKAT: PROVISION, PAID
    # OTHER: MISC
    sub_types = "PAYMENT,MOTOR,BHUSSA,GAWALI,CASH_ADV,OTHER,RECEIPT,TAKEN,WITHDRAWN,REPAID,GIVEN,RECEIVED,MANDI,TRAVEL,FOOD,SALARY,BF_DISC,JB_PAID,PROVISION,PAID,MISC"
    ...
    for c, w in enumerate([4, 11, 10, 12, 20, 20, 12, 12, 12, 12, 2, 14, 14], 1):
        col_w(ws, c, w)
    ...
    ws.cell(row=2, column=1, value="From: Daily_Sales + Cash_Book (Type=BEPAARI)").font = Font(italic=True, size=9)
    ...
    ws.cell(row=2, column=1, value="From: Daily_Sales + Cash_Book (Type=DUKANDAR)").font = Font(italic=True, size=9)
    ...
    tot = 81
    ws.cell(row=tot, column=1, value="TOTAL").font = Font(bold=True)
    ws.cell(row=tot, column=2, value="PATTI (Total O/S)")
    ...
    # ================================================================
    # 7. BALANCE_SHEET (FIXED: Zakat as Liability, BF_DISC as Expense)
    # ================================================================
    ...
    ws.cell(row=6, column=1, value="  Opening + Received")
    # Capital = Opening from Masters + CAPITAL TAKEN from Cash_Book - CAPITAL WITHDRAWN
    ws.cell(row=6, column=2, value='=SUM(Masters!$P$3:$P$24)+SUMIFS(Cash_Book!$I:$I,Cash_Book!$C:$C,"CAPITAL",Cash_Book!$D:$D,"TAKEN")+SUMIFS(Cash_Book!$J:$J,Cash_Book!$C:$C,"CAPITAL",Cash_Book!$D:$D,"TAKEN")-SUMIFS(Cash_Book!$G:$G,Cash_Book!$C:$C,"CAPITAL",Cash_Book!$D:$D,"WITHDRAWN")-SUMIFS(Cash_Book!$H:$H,Cash_Book!$C:$C,"CAPITAL",Cash_Book!$D:$D,"WITHDRAWN")')
    ws.cell(row=6, column=2).number_format = MONEY_FMT
    
    # LOANS/AMANAT
    ws.cell(row=8, column=1, value="LOANS/AMANAT:").font = Font(bold=True)
    ws.cell(row=9, column=1, value="  Taken - Repaid")
    ...
    # BEPAARI PAYABLES
    ws.cell(row=11, column=1, value="BEPAARI PAYABLES:").font = Font(bold=True)
    ws.cell(row=12, column=1, value="  From Ledger")
    ws.cell(row=12, column=2, value='=Bepaari_Ledger!R82')
    ws.cell(row=12, column=2).number_format = MONEY_FMT
    
    # COMMISSION EARNED (Net)
    ws.cell(row=14, column=1, value="COMMISSION (Net):").font = Font(bold=True)
    ws.cell(row=15, column=1, value="  Gross - Discounts")
    ws.cell(row=15, column=2, value='=Bepaari_Ledger!G82-Dukandar_Ledger!E81')
    ws.cell(row=15, column=2).number_format = MONEY_FMT
    
    # JB COLLECTED
    ws.cell(row=17, column=1, value="JB COLLECTED:")
    ws.cell(row=17, column=2, value='=Bepaari_Ledger!I82')
    ws.cell(row=17, column=2).number_format = MONEY_FMT
    
    # KK COLLECTED
    ws.cell(row=18, column=1, value="KK COLLECTED:")
    ws.cell(row=18, column=2, value='=Bepaari_Ledger!H82')
    ws.cell(row=18, column=2).number_format = MONEY_FMT
    
    # ZAKAT PAYABLE (Opening + Provisioned - Paid)
    ws.cell(row=20, column=1, value="ZAKAT PAYABLE:").font = Font(bold=True)
    ws.cell(row=21, column=1, value="  Opening + Provision - Paid")
    # Zakat = Opening (Masters) + ZAKAT PROVISION - ZAKAT PAID
    ws.cell(row=21, column=2, value='=Masters!$X$9+SUMIFS(Cash_Book!$I:$I,Cash_Book!$C:$C,"ZAKAT",Cash_Book!$D:$D,"PROVISION")+SUMIFS(Cash_Book!$J:$J,Cash_Book!$C:$C,"ZAKAT",Cash_Book!$D:$D,"PROVISION")-SUMIFS(Cash_Book!$G:$G,Cash_Book!$C:$C,"ZAKAT",Cash_Book!$D:$D,"PAID")-SUMIFS(Cash_Book!$H:$H,Cash_Book!$C:$C,"ZAKAT",Cash_Book!$D:$D,"PAID")')
    ws.cell(row=21, column=2).number_format = MONEY_FMT
    
    # Total Liabilities
    ws.cell(row=23, column=1, value="TOTAL LIABILITIES").font = Font(bold=True)
    ws.cell(row=23, column=2, value='=B6+B9+B12+B15+B17+B18+B21')
    ws.cell(row=23, column=2).number_format = MONEY_FMT
    ws.cell(row=23, column=2).font = Font(bold=True)
    ws.cell(row=23, column=2).fill = CALC_FILL
    ...
    ws.cell(row=13, column=4, value="  Given - Received")
    ...
    # EXPENSES (including BF_DISC as expense)
    ...
    ws.cell(row=17, column=4, value="  BF Discount (Expense)")
    ...
    ws.cell(row=18, column=5, value='=SUMIFS(Cash_Book!$G:$G,Cash_Book!$C:$C,"EXPENSE",Cash_Book!$D:$D,"JB_PAID")+SUMIFS(Cash_Book!$H:$H,Cash_Book!$C:$C,"EXPENSE",Cash_Book!$D:$D,"JB_PAID")')
    ws.cell(row=18, column=5).number_format = MONEY_FMT
    
    # Total Assets
    ws.cell(row=23, column=4, value="TOTAL ASSETS").font = Font(bold=True)
    ws.cell(row=23, column=5, value='=E5+E7+E10+E13+E16+E17+E18')
    ws.cell(row=23, column=5).number_format = MONEY_FMT
    ws.cell(row=23, column=5).font = Font(bold=True)
    ws.cell(row=23, column=5).fill = CALC_FILL
    
    # Difference
    ws.cell(row=25, column=1, value="DIFFERENCE (Should be 0)").font = Font(bold=True, color="FF0000")
    ws.cell(row=25, column=2, value='=B23-E23')
    ws.cell(row=25, column=2).number_format = MONEY_FMT
    ws.cell(row=25, column=2).font = Font(bold=True, color="FF0000")
    ...
    items = [
        ...
        ["", ""],
        ["EXPENSES:", ""],
        ["  Mandi/Business", '=SUMIFS(Cash_Book!$G:$G,Cash_Book!$C:$C,"EXPENSE",Cash_Book!$D:$D,"MANDI")+SUMIFS(Cash_Book!$H:$H,Cash_Book!$C:$C,"EXPENSE",Cash_Book!$D:$D,"MANDI")+SUMIFS(Cash_Book!$G:$G,Cash_Book!$C:$C,"EXPENSE",Cash_Book!$D:$D,"TRAVEL")+SUMIFS(Cash_Book!$H:$H,Cash_Book!$C:$C,"EXPENSE",Cash_Book!$D:$D,"TRAVEL")+SUMIFS(Cash_Book!$G:$G,Cash_Book!$C:$C,"EXPENSE",Cash_Book!$D:$D,"FOOD")+SUMIFS(Cash_Book!$H:$H,Cash_Book!$C:$C,"EXPENSE",Cash_Book!$D:$D,"FOOD")+SUMIFS(Cash_Book!$G:$G,Cash_Book!$C:$C,"EXPENSE",Cash_Book!$D:$D,"SALARY")+SUMIFS(Cash_Book!$H:$H,Cash_Book!$C:$C,"EXPENSE",Cash_Book!$D:$D,"SALARY")+SUMIFS(Cash_Book!$G:$G,Cash_Book!$C:$C,"EXPENSE",Cash_Book!$D:$D,"OTHER")+SUMIFS(Cash_Book!$H:$H,Cash_Book!$C:$C,"EXPENSE",Cash_Book!$D:$D,"OTHER")'],
        ["  BF Discount (Expense)", '=SUMIFS(Cash_Book!$G:$G,Cash_Book!$C:$C,"EXPENSE",Cash_Book!$D:$D,"BF_DISC")+SUMIFS(Cash_Book!$H:$H,Cash_Book!$C:$C,"EXPENSE",Cash_Book!$D:$D,"BF_DISC")'],
        ["  JB Paid to Market", '=SUMIFS(Cash_Book!$G:$G,Cash_Book!$C:$C,"EXPENSE",Cash_Book!$D:$D,"JB_PAID")+SUMIFS(Cash_Book!$H:$H,Cash_Book!$C:$C,"EXPENSE",Cash_Book!$D:$D,"JB_PAID")'],
    ...
    # ================================================================
    wb.save(path)
    print(f"✅ Mandi Master V5 created: {path}")
    return path


if __name__ == "__main__":
    create_v5("/app/Mandi_Master_V5.xlsx")
